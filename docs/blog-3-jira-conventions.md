...
- **Front-end:** Adaptive [`ImageWatermarkOverlay`](../packages/components/src/ui/image-watermark-overlay.tsx) samples a ≤64px CDN proxy of the photo corner; luminance &lt; 0.45 → light mark, else dark. [`WatermarkProvider`](../packages/components/src/ui/watermark-context.tsx) carries both URLs. Geometry: 12% width, bottom-right, 5% padding.
