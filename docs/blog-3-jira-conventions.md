...
| [PROD-2200](https://dotdirect.atlassian.net/browse/PROD-2200) | Favicon in Studio Global Settings                           | In Development                                                                                                               | `settings.favicon` ([`settings.ts`](../apps/studio/schemas/settings.ts)) → [`buildFaviconIcons`](../packages/sanity/src/favicon.ts) → blog + www `generateMetadata`; bundled `.ico` moved to `public/`                                                                                                                                                |
...
- **Jira:** [PROD-2200](https://dotdirect.atlassian.net/browse/PROD-2200). Branch: `feat/ziming-sanity`.
- **Studio:** `favicon` image field on [`settings.ts`](../apps/studio/schemas/settings.ts) (Identity & brand). Accepts PNG / SVG / WebP — Sanity cannot store `.ico`; validation warns on non-square or <48px.
...
- **Raster vs SVG:** raster is CDN-resized (32 / 192 / 180 apple-touch); SVG is served as-is (Sanity does not transform SVG) with no iOS touch icon.
