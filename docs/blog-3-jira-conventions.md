...
| [PROD-2206](https://dotdirect.atlassian.net/browse/PROD-2206) | Image watermark (website & blog)                            | In Development                                                                                                               | Render-time overlay: [`packages/components/src/ui/image-watermark-overlay.tsx`](../packages/components/src/ui/image-watermark-overlay.tsx); Studio `settings.watermark` + per-image `applyWatermark`; blog/www `SanityImage` + layouts                                                                                                                      |
...
- **Studio:** Global Settings → Identity → `watermark` (`enabled`, `image`, `opacity`). Per-image `Show watermark` (`applyWatermark`, default true) on `bodyImage` and gallery images only. **Not** on post hero/`mainImage`, cards, category banners, video thumbs, case study hero/card, product fields, or `ogImage`.
- **Front-end:** [`ImageWatermarkOverlay`](../packages/components/src/ui/image-watermark-overlay.tsx) + [`WatermarkProvider`](../packages/components/src/ui/watermark-context.tsx); blog/www layouts fetch via `BLOG_GLOBAL_SETTINGS_QUERY`; body/gallery `SanityImage` watermarks when enabled. Cards, heroes, banners, video thumbs, products, author photos force `applyWatermark={false}`. Geometry: 12% width, bottom-right, 5% padding.
- **Bake-on-serve trial:** `NEXT_PUBLIC_WATERMARK_MODE=serve` → editorial images via `GET /api/wm` (Sharp). Revert with `overlay` or unset. Details: [`apps/blog/memory.md`](../apps/blog/memory.md) § PROD-2206.
- **Human ops:** upload ticket SVG `pakfactory_white-logo_watermark.svg` into Global Settings → Watermark image (agents do not write documents).
- **Out of scope:** video frame watermarking, OG URL watermarks, bake-on-upload.
- **Verify:** `pnpm --filter @pakfactory/blog typecheck`; www typecheck; Studio settings + per-image toggle; product PDP unmarked.