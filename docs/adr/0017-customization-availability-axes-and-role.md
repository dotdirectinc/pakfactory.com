...
- **Sanity's single `Coating` Type spans two of Eric's Types.** The diagram splits it into **Spot Coating** (un-badged: *Spot UV/Spot Gloss*, *Spot Glitter*, *Raised Spot UV*, *Textured Spot UV*) and **Surface Coating** (*"Not Customizable"*: UV, AQ (Aqueous), Varnish, Soft-Touch, …). Because both live under one Sanity Type today, the classification is enumerated per-slug rather than derived from the Type. Splitting the Type is separate work and is not ticketed.
