...
| **Sanity migration register** | `studio` (+ datasets) | **021** (ledger in dataset, probe per migration) | **021** |
...
| **Redirects** | `blog` | — | 003 |
| **Product customization model** | `www`, `studio` | **022** (two fields not four; empty **fails closed**; Sanity owns the rules) | 017 → **022** |
| **Admin search** | `admin` (+ content sync) | [ADR-018](0018-admin-search-foundation.md) | 018 |
| **Workspace packages** | `all` | [`AGENTS.md`](../../AGENTS.md) § Workspace packages | **019** |
...
| 021 | **Sanity migration register** — applied-migration state is a `migrationRun` document **in each dataset**, not a file in git; every one-shot migration carries a GROQ **probe** asserting the old shape is gone, so `status` flags ledger/reality disagreement instead of assuming; repeatable tasks stay out of the register; scripts consolidate under `scripts/sanity/` split one-shot vs repeatable, superseding the placement rule's content-model vs operational axis. | Accepted | `studio` | [`0021-sanity-migration-register.md`](0021-sanity-migration-register.md) |
| 022 | **Customization rules are computed, not stored** — the four ADR-017 availability fields retired for `product.availableCustomizations` + `customizationOption.compatibleCustomizations`, whose **empty fails closed** (reversing D47); Sanity owns the rules and the registry becomes a history log; `availabilityDecidedBy` discriminates, `dependsOn` carries direction and is derived from the approved registry rules; one shared package computes for Studio/www/admin. Availability = direct ∪ derived (derived displayed, never stored) and `customizationExceptions` overrides per product both ways — both implemented (PROD-2595), awaiting Eric's confirmation. | **Proposed**; decisions 1–5 shipped, 6–7 implemented | `www`, `studio` | [`0022-customization-rules-computed-not-stored.md`](0022-customization-rules-computed-not-stored.md) |
