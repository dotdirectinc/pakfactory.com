...
> **Terminology (revised 2026-06-17): "section", not "block".** This ADR went through an interim "block" naming (`components/blocks/`, `BlockRenderer`, "Page blocks") to mirror turbo-start-sanity; it was reversed back to **"section"** to match the Next.js code element — `components/sections/` ⇄ Studio `schemas/sections/`, `SectionRenderer`/`SECTION_COMPONENTS`, Studio label "Page sections". Sanity **content field names** (`pageBuilder`, the `_type`s like `postFeaturedRow`) were left unchanged — no content migration.

## Context

ADR-005 grouped `src/components/` by **feature/domain** (`post/ category/ tag/ author/ home/ …` + `common/`). As the blog moves to an editor-driven **page-builder homepage** (one Sanity `pageBuilder` array of page-builder sections, each rendered by a matching React component), a different organizing axis became more useful in practice:

- The **page-builder sections** are a first-class concept that must mirror the Studio [`schemas/sections/`](../../apps/studio/schemas/sections/) folder 1:1 (the `_type → component` resolver depends on this correspondence). They deserve a dedicated home, not dispersal across feature folders.
- The reference implementations we follow — **`robotostudio/turbo-start-sanity`** (`schemaTypes/blocks/` + `components/sections/` on web) and **Sanity Ignite** (`sections/` + `templates/` + `modules/` + `ui/`) — use role-based folders for page-builder sections, route templates, data-driven modules, and presentational primitives. We align Studio and web on **section** terminology (`schemas/sections/` ↔ `components/sections/`).
...
|--------|-------|----------|
| `sections/` | **Page-builder sections** — one per Studio `schemas/sections/` type; targets of the `_type → component` resolver (`SectionRenderer` + `SECTION_COMPONENTS`). | `post-featured-row`, `post-category-row`, `post-spotlight-row`, `tag-strip`, `cta-newsletter`, `cta-rfq`, `cta-pillars`, `rich-text-band` |
...
1. Editor places it in the Sanity page builder? → **`sections/`**
...
- **Naming (ADR-005 D5):** kebab-case files, **file name === exported component** (`post-card.tsx` ↔ `PostCard`). Never a registry/design-tool ID (`hero-section-32`). A section file is named for the **section stem** (`post-featured-row` → `PostFeaturedRow`), not a positional ID.
...
**Page-builder sections** (in `sections/`):
...
Prefixes group intent (`post` = post-driven rows, `cta` = conversion blocks, `tag` = tag navigation, `filter` = archive faceting). Adding a page-builder section requires Studio schema in `schemas/sections/` + component in `components/sections/` + `registry.ts` entry + GROQ branch in the page-builder query.
...
ADR-005 chose feature/domain on the "screaming architecture" argument (UI should shout what it does for the product). We are overriding that **specifically because the page-builder makes architectural role the dominant axis**: the build wires sections → `SectionRenderer` → chrome, and grouping by role keeps `sections/` aligned with Studio `schemas/sections/` and keeps page-builder sections from scattering. The accepted cost: a single feature's pieces (e.g. everything "tag") are now spread across `sections/`, `views/`, `modules/`, and `ui/` rather than co-located in `tag/`. For a CMS-page-builder app this is the better trade; for a non-CMS app it may not be, so **scope is `apps/blog` only** and `apps/www` is free to choose its own axis when remediated.
...
- `sections/` mirrors [`apps/studio/schemas/sections/`](../../apps/studio/schemas/sections/) 1:1. Adding a page-builder section: Studio schema in `schemas/sections/` + component in `components/sections/` + `registry.ts` + GROQ branch.
- Studio field labels use **Page sections**; Sanity content field names (`pageBuilder`, section `_types`) stay stable — no content migration.
