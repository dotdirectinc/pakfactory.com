...
**Narrowing:** every pick goes to `resolveWithSelections` as-is. A Type's list is narrowed by the *other* Types' picks, never its own; a Type not answered yet still counts as possible (Color System stays pickable before a Printing Method is chosen). Anything listed can be picked and stays picked; a later pick that makes an earlier one impossible clears just that pick, silently.
