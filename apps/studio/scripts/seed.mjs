...
const productStyleCategories = [
  // Folding Carton
  { _id: 'psc-ste',  _type: 'productStyle', title: 'Straight Tuck End',     slug: slug('straight-tuck-end'),    productCategories: [ref('pc-folding-carton')], description: 'Both tuck flaps fold in the same direction — front closes at top, back at bottom.', order: 1 },
  { _id: 'psc-rte',  _type: 'productStyle', title: 'Reverse Tuck End',      slug: slug('reverse-tuck-end'),     productCategories: [ref('pc-folding-carton')], description: 'Tuck flaps fold in opposite directions — front closes at top, back at top.', order: 2 },
  { _id: 'psc-auto', _type: 'productStyle', title: 'Auto Bottom (123)',      slug: slug('auto-bottom'),          productCategories: [ref('pc-folding-carton')], description: 'Pre-glued base that auto-locks on opening — faster assembly, stronger base.', order: 3 },
  // Rigid
  { _id: 'psc-mag',  _type: 'productStyle', title: 'Magnetic Closure',      slug: slug('magnetic-closure'),     productCategories: [ref('pc-rigid')],         description: 'Hinged lid held shut by embedded magnets — signature luxury unboxing format.', order: 1 },
  { _id: 'psc-neck', _type: 'productStyle', title: 'Neck Box',              slug: slug('neck-box'),             productCategories: [ref('pc-rigid')],         description: 'Two-piece box with a raised inner tray (neck) that slides into the outer shell.', order: 2 },
  { _id: 'psc-tele', _type: 'productStyle', title: 'Telescoping Box',       slug: slug('telescoping-box'),      productCategories: [ref('pc-rigid')],         description: 'Deep lid slides over a deep base — equal-depth two-piece construction.', order: 3 },
  // Mailer
  { _id: 'psc-rsc',  _type: 'productStyle', title: 'Regular Slotted Container', slug: slug('regular-slotted-container'), productCategories: [ref('pc-mailer')], description: 'Standard corrugated shipper with four equal flaps meeting at center.', order: 1 },
  { _id: 'psc-snap', _type: 'productStyle', title: 'Snap-Lock Mailer',      slug: slug('snap-lock-mailer'),     productCategories: [ref('pc-mailer')],        description: 'Tool-free self-locking base with hinged lid — optimized for e-commerce.', order: 2 },
]
