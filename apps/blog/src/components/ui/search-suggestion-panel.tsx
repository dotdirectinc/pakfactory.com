...
export function SearchSuggestionPanel({
  ...
  return (
    <div
      className={cn(
        "absolute top-[calc(100%+0.375rem)] right-0 left-auto z-50 min-w-full w-[28rem] max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-border bg-background shadow-md",
        className,
      )}
    >
      <SearchSuggestTabs
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-foreground">
                      <SearchHighlight text={item.title} query={trimmedQuery} />
                    </span>
                    {item.subtitle ? (
