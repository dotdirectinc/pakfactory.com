...
export function PostFaqSection({ items }: PostFaqSectionProps) {
  ...
  return (
    <section className="mt-16 border-t border-dashed border-border pt-16" aria-labelledby="post-faq-heading">
      <h2 id="post-faq-heading" className="text-2xl font-semibold tracking-tight text-foreground">
          ...
          >
            ...
            <CollapsibleContent className="pb-4 text-muted-foreground">
              {item.answer?.length ? (
                ...
                <p className="text-sm leading-6">{item.answerText}</p>
              )}
            </CollapsibleContent>
          </Collapsible>
        ))}
