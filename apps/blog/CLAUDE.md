...
- **Dieline gutters:** mobile **outer 16px** (`px-4`) + **inner 48px** (`px-12`) = **64px total** viewport → content (including border); desktop **outer 32px** (`md:px-8`) + **inner 64px** (`sm:px-16`) via helpers in [`page-dieline-section.tsx`](./src/components/layout/page-dieline-section.tsx). Do not set mobile outer to `px-8`. Prefer helpers; flush borders need `px-0 sm:px-0` (bare `px-0` does not clear `sm:px-16`). Full-bleed bands must sit in **exactly one** outer wrapper so negative margins cancel once. Newsletter cream uses a **`w-screen` viewport shell** with content in outer+inner — render it outside padded rails.
