...
- **Dieline gutters:** tokens `--layout-gutter-outer` / `--layout-gutter-inner` (`px-layout-gutter-*`). Mobile **outer 16 + inner 16** = **32px total** viewport → content; `sm+` inner **32px**; `md+` outer **32px** (matches `gap-8`) via helpers in [`page-dieline-section.tsx`](./src/components/layout/page-dieline-section.tsx). Do not set mobile outer to `px-8`. Prefer helpers; flush borders need `px-0` (twMerge clears the token utility). Full-bleed bands must sit in **exactly one** outer wrapper so negative margins cancel once. Newsletter cream uses a **`w-screen` viewport shell** with content in outer+inner — render it outside padded rails.
