...
- **Production (launch):** `pakfactory.com/blog` — the main-site **CloudFront** distribution routes `/blog*` to the Vercel origin (`backup.blog.pakfactory.com`) as a second origin. CloudFront must send the *origin's* Host header (managed policy `AllViewerExceptHostHeader`), respect origin cache-control, forward query strings, and include `Accept` in the cache key for `/blog/_next/image*`. Root `robots.txt` is Magento's — add `Sitemap: https://www.pakfactory.com/blog/sitemap.xml` there. (The multi-zones approach documented earlier doesn't apply: the root domain is Magento, not a Next app.)
