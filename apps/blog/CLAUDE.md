...
| `/feed`, `/*/feed`         | `next.config.ts` `redirects()`                                                               | PROD-2140: legacy WordPress feeds (main + per-category/tag/author/comment/post) **301 → `/rss.xml`**. Placed above the `/category/*` rules so `/category/{c}/feed` isn't swallowed first. |
| `/sitemap.xml`             | [`src/app/(sitemaps)/sitemap.xml/route.ts`](./src/app/%28sitemaps%29/sitemap.xml/route.ts)                             | Sitemap index (PROD-1865); lists all sub-sitemaps; `revalidate = 300`                                                   |
| `/pages-sitemap.xml`       | [`src/app/(sitemaps)/pages-sitemap.xml/route.ts`](./src/app/%28sitemaps%29/pages-sitemap.xml/route.ts)                 | Non-post indexable pages: `/`, `/all` (posts archive index), `/contribute`, + CMS landing/static pages                  |
| `/categories-sitemap.xml`  | [`src/app/(sitemaps)/categories-sitemap.xml/route.ts`](./src/app/%28sitemaps%29/categories-sitemap.xml/route.ts)       | Indexable categories only (PROD-2133): drops `allowIndex == false` and, when `categoryDefaults.hideEmptyCategory` is on, **0-post** categories — same `isCategoryHiddenAsEmpty` rule as the page meta-robots |
| `/authors-sitemap.xml`     | [`src/app/(sitemaps)/authors-sitemap.xml/route.ts`](./src/app/%28sitemaps%29/authors-sitemap.xml/route.ts)             | Authors with ≥1 published post                                                                                          |
| `/posts-sitemap`           | [`src/app/(sitemaps)/posts-sitemap/route.ts`](./src/app/%28sitemaps%29/posts-sitemap/route.ts)                         | 301 → `/posts-sitemap-1.xml`                                                                                            |
| `/posts-sitemap-[n].xml`   | [`src/app/(sitemaps)/posts-sitemap/[page]/route.ts`](./src/app/%28sitemaps%29/posts-sitemap/%5Bpage%5D/route.ts)       | Paginated **post URLs only** (200/group) + featured-image sitemap. Public `-[n].xml` (rewrite in `next.config.ts`) → handler; internal `/posts-sitemap/[n]` still accepted |
| `/topics-sitemap`            | [`src/app/(sitemaps)/topics-sitemap/route.ts`](./src/app/%28sitemaps%29/topics-sitemap/route.ts)                           | 301 → `/topics-sitemap-1.xml`                                                                                             |
| `/topics-sitemap-[n].xml`    | [`src/app/(sitemaps)/topics-sitemap/[page]/route.ts`](./src/app/%28sitemaps%29/topics-sitemap/%5Bpage%5D/route.ts)         | Paginated topics with ≥1 post (200/group). Public `-[n].xml` (rewrite) → handler; internal `/topics-sitemap/[n]` still accepted. Legacy `/tags-sitemap*` → 301 |
...
- **Base path is env-driven (2026-07-15):** `next.config.ts` reads `basePath` from **`NEXT_PUBLIC_BLOG_BASE_PATH`** — the same variable `src/lib/site.ts` uses for canonicals/JSON-LD/RSS/sitemaps, so **one env var flips the whole app**. Unset (staging, previews, local) → origin-root. Set `NEXT_PUBLIC_BLOG_BASE_PATH=/blog` (+ `NEXT_PUBLIC_SITE_URL=https://www.pakfactory.com`) on the production Vercel project → everything serves and self-references under `/blog`.
- **Production (launched 2026-07-15):** `pakfactory.com/blog` — the Magento boxes' **nginx** proxies `location ^~ /blog` → the Vercel origin **`origin.blog.pakfactory.com`** (custom domain; the project's `.vercel.app` URL is SSO-gated). `blog.pakfactory.com` 301s to `/blog` at the ALB; the old WordPress blog is archived at `backup.blog.pakfactory.com`. Runbook: `docs/` release runbooks.
- **Domain-root files (RFC 9309 — per-host, root-only):** the public `robots.txt` is **Magento's** (carries the blog + case-studies `Sitemap:` lines); the public `llms.txt` is **generated by `apps/www`** at the domain root (aggregates blog posts/categories + case studies from the shared dataset). The blog app deliberately serves **no** `robots.txt`/`llms.txt` of its own — under `basePath` they could only appear at `/blog/…`, which crawlers ignore.
...
- **`getSiteUrl()`** / **`NEXT_PUBLIC_SITE_URL`** is the **origin only** (scheme + host, no path), e.g. `http://localhost:3004`, `https://blog.pakfactory.com`. `siteBaseUrl()` = origin + `BLOG_BASE_PATH`.
- **`getWwwUrl()`** — main marketing site for organization JSON-LD and industry links (`NEXT_PUBLIC_WWW_URL`); **not** blog-prefixed.
- **Local dev:** [http://localhost:3004](http://localhost:3004) (default `PORT=3004`). Ops, env, seed: [`memory.md`](./memory.md) § Local dev.
...
Agent practice summary (RSC / state / placement checklist): **[`ENGINEERING.md`](../../ENGINEERING.md)**. Detail below stays binding for blog.
...
- **Dieline gutters:** tokens `--layout-gutter-outer` / `--layout-gutter-inner` (`px-layout-gutter-*`). Mobile **outer 16 + inner 16** = **32px total** viewport → content; `sm+` inner **80px** (matches section `sm:py-20`); `md+` outer **32px** via helpers in [`page-dieline-section.tsx`](./src/components/layout/page-dieline-section.tsx). Do not set mobile outer to `px-8`. Prefer helpers; flush borders need `px-0` (twMerge clears the token utility). Full-bleed bands must sit in **exactly one** outer wrapper so negative margins cancel once. Newsletter cream uses a **`w-screen` viewport shell** with content in outer+inner — render it outside padded rails.
