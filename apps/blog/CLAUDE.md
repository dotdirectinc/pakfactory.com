...
- **Dieline gutters:** mobile **outer 16px** (`px-4`) + **inner 16px** (`px-4`) = **32px total** viewport → content (including border) via helpers in [`page-dieline-section.tsx`](./src/components/layout/page-dieline-section.tsx). Do not set mobile outer to `px-8`. Prefer helpers; flush borders need `px-0 md:px-0` (bare `px-0` does not clear `md:px-8`). Full-bleed bands must sit in **exactly one** outer wrapper so negative margins cancel once. Newsletter cream uses a **`w-screen` viewport shell** with content in outer+inner — render it outside padded rails.
