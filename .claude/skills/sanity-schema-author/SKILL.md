...
- **www Sections (ADR-020):** route gate + D35 before adding a type. **§8** — if a curated list already lives on the page document, the section field is an **optional override** (no duplicate `min`). **§10 insert UX** — place the type in an **entity** insert tab (Solutions / Case studies / Products / Customizations / Expertise / Resources / Clients / Layout / CTAs). Studio `title`: `{Entity} row` when using `rowSectionFields()`, else a plain job name; never casual `_type` renames (ADR-014). Wire `SectionItemPreview`, distinct icon, optional thumbnail, **`groups: sectionFieldGroups()`** (All · Heading · Content · Layout), and tag payload with `SECTION_GROUPS.content`. Heading/intro/`link.query` use token chips (`SectionTokenStringInput`; include `%slug%` for catalog query prefilter). Full ship checklist: ADR-020 §11.
